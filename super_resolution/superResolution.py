...
def prepareModel(input_path_, model_path_):
    ...
    checkoutpoint = torch.load(model_path_, map_location=torch.device('cpu'))
    net = SRResNet().to('cpu')     # 用cuda加速测试，也可以不用，不用cuda加速测试速度会很慢
    net.load_state_dict(checkoutpoint['model'])      # 加载训练好的模型参数
    net.eval()      # 将模型切换到测试模式
